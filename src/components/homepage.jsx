...
import logo from '../assets/nova_logo.png'; // Adjust the path to your logo image
...
useEffect(() => {
  ...
  return (
    ...
    <div style={{ backgroundColor: '#000', color: 'pure white', fontFamily: 'Arial, sans-serif' }}>
      {/* Spotlight effect */}
<div
  ref={spotlightRef}
  style={{
...
>
  ...
  {['home', 'talents', 'gallery', 'about', 'team', 'testimonials', 'contact', 'login'].map((id) => (
    <a
      key={id}
      href={`#${id}`}
      style={{
  ...
  {[
    ...
    >
      ...
      <section style={gridSectionStyle}>
        {[
          ['Shanmathi Vijay', 'Creative Director'],
          ['Karthik Raj', 'Project Manager'],
...
>
  ...
  <p style={{ marginTop: 20, textAlign: 'center', color: '#ccc' }}>
    ...
    >
      ...
      {/* Footer Slider */}
      <div style={{ background: '#0a0a0a', padding: '60px 0', overflow: 'hidden' }}>
        {['right', 'left'].map((dir, idx) => (
          <div key={dir} style={{
            display: 'flex', animation: `scroll-${dir} 30s linear infinite`,
            gap: '40px', padding: dir === 'right' ? '0 0 0 100%' : '0 100% 0 0',
            marginTop: idx ? '40px' : '0'
          }}>
            {['Editorials', 'Events', 'Animation', 'Lookbooks', 'Music Videos'].map((cat, i) => (
              <div key={i} style={{ minWidth: '180px', textAlign: 'center' }}>
                <img src={`https://via.placeholder.com/150x140?text=${encodeURIComponent(cat)}`} alt={cat}
                  style={{ borderRadius: '12px', width: '100%', height: '140px', objectFit: 'cover' }} />
                <h3 style={{ color: 'white', fontWeight: 500 }}>{cat}</h3>
              </div>
            ))}
          </div>
        ))}
      ...
      <footer data-aos="fade-up" style={{ background: '#000', textAlign: 'center', padding: '40px', fontSize: '0.9rem', color: '#777' }}>
        &copy; 2025 Nova Film Industry. All rights reserved.
      </footer>
